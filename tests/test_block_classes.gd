...
func test_blocks_builtin_operands():
	var a_vector2 = BlockLiteral.new(Block.Category.MATH, "Vector2(100, 100)")
	assert_eq(a_vector2.category, Block.Category.MATH)
	...
	var wrong = BlockLiteral.new(Block.Category.MATH, "#%%!!")
	...
	var b_vector2 = BlockLiteral.new(Block.Category.MATH, "Vector2(200, 0)", TYPE_VECTOR2)
	assert_eq(b_vector2.get_type(), TYPE_VECTOR2)

	var a_plus_b = BlockExpression.new(Block.Category.MATH, "{a} + {b}")
	...
	assert_true(a_plus_b.can_bind_argument('a', BlockLiteral.new(Block.Category.MATH, "1.1", TYPE_FLOAT)))
	...
	a_plus_b.bind_arguments({
		'a': a_vector2,
		'b': BlockLiteral.new(Block.Category.MATH, "123"),
	})
	assert_true(a_plus_b.has_errors())
	assert_eq(a_plus_b.get_error_message(), "Invalid argument types.")
	assert_eq(a_plus_b.get_type(), TYPE_NIL)

	var a_plus_b_divided = BlockExpression.new(Block.Category.MATH, "{dividend} / {divisor}")
	...
	var c_vector2 = BlockLiteral.new(Block.Category.MATH, "Vector2(1, 2)")
	a_plus_b.bind_arguments({'a': a_vector2, 'b': b_vector2})
	...
	a_plus_b_divided.bind_arguments({
		'dividend': BlockLiteral.new(Block.Category.MATH, "123"),
		'divisor': c_vector2,
	})
	...
	var length_squared = BlockExpression.new(Block.Category.MATH, "{this}.length_squared()")
	...
	var lerp_block = BlockExpression.new(Block.Category.MATH, "{this}.lerp({to}, {weight})")
	lerp_block.argument_types = {		{'this': TYPE_VECTOR2, 'to': TYPE_VECTOR2, 'weight': TYPE_FLOAT}: TYPE_VECTOR2,
	}
	a_plus_b.bind_arguments({'a': a_vector2, 'b': b_vector2})
	lerp_block.bind_arguments({'this': a_plus_b, 'to': c_vector2, 'weight': BlockLiteral.new(Block.Category.MATH, "0.5")})
